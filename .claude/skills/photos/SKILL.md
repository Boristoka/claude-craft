---
name: photos
description: Stock photo URLs organized by category for use in websites.
---

# Stock Photos Library

Quality Unsplash photos organized by category. All URLs include optimal sizing parameters.

---

## Hero / Office Spaces

```
https://images.unsplash.com/photo-1497366216548-37526070297c?w=1920&q=85
https://images.unsplash.com/photo-1497215842964-222b430dc094?w=1920&q=85
https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=1920&q=85
https://images.unsplash.com/photo-1497366811353-6870744d04b2?w=1920&q=85
https://images.unsplash.com/photo-1504384308090-c894fdcc538d?w=1920&q=85
```

---

## Team / Collaboration

```
https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=1920&q=85
https://images.unsplash.com/photo-1551434678-e076c223a692?w=1200&q=85
https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=1200&q=85
https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?w=1200&q=85
```

---

## Portraits (for team/testimonials)

**Professional headshots:**
```
https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&q=80
https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=400&q=80
https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=400&q=80
https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=400&q=80
https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=400&q=80
https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=400&q=80
https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=400&q=80
https://images.unsplash.com/photo-1580489944761-15a19d654956?w=400&q=80
```

---

## Abstract / Minimal

```
https://images.unsplash.com/photo-1561070791-2526d30994b5?w=1200&q=85
https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=1200&q=85
https://images.unsplash.com/photo-1557683316-973673baf926?w=1200&q=85
https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=1920&q=85
```

---

## Industry-Specific

### Restaurant / Food
```
https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=1920&q=85
https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=1200&q=85
https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=1200&q=85
```

### Architecture / Real Estate
```
https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=1920&q=85
https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?w=1200&q=85
https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=1200&q=85
```

### Healthcare / Wellness
```
https://images.unsplash.com/photo-1576091160550-2173dba999ef?w=1200&q=85
https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=1200&q=85
```

### Legal / Finance
```
https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=1200&q=85
https://images.unsplash.com/photo-1554224155-6726b3ff858f?w=1200&q=85
```

### Technology
```
https://images.unsplash.com/photo-1518770660439-4636190af475?w=1200&q=85
https://images.unsplash.com/photo-1451187580459-43490279c0fa?w=1920&q=85
```

### Fitness / Sports
```
https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=1200&q=85
https://images.unsplash.com/photo-1571902943202-507ec2618e8f?w=1200&q=85
```

### Education
```
https://images.unsplash.com/photo-1523050854058-8df90110c9f1?w=1200&q=85
https://images.unsplash.com/photo-1427504494785-3a9ca7044f45?w=1200&q=85
```

### Retail / Shopping
```
https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=1200&q=85
https://images.unsplash.com/photo-1472851294608-062f824d29cc?w=1200&q=85
```

---

## Photo Selection Tips

1. **Neutral colors** - Avoid bright, saturated images
2. **Professional settings** - Real offices, meetings, workspaces
3. **No clichés** - Skip "people pointing at laptops" etc.
4. **Consistent mood** - Same lighting style throughout site
5. **High resolution** - Use `w=1920` for heroes, `w=1200` for sections
6. **Quality parameter** - Always include `q=85` for good compression

---

## URL Parameters

| Parameter | Usage |
|-----------|-------|
| `w=1920` | Hero images (full-width) |
| `w=1200` | Section images |
| `w=400` | Avatars/thumbnails |
| `q=85` | Quality (good balance) |
| `q=80` | Smaller file size |
